...
Тестовые заглушки (_Test doubles_) - Это общий термин для группы объектов, которые служат для замены production-сущностей при тестировании. Самый простой пример - использование локальной базы данных. Всего их 5: **Dummy**, **Fake**, **Stub**, **Mock**, **Spy**.
...
Максимально простой тест. Создаём тестовый класс, который не надо помечать аттрибутами и который не наследуется от каких-то библиотечны классов. Тесты помечаются аттрибутом `Fact`. Для проверок используется класс `Assert` с кучей статических методов. Конечно, мы можем обойтись одним методом `Assert.True()`, который проверяет, что переданное в него выражение истино, но если тест не прошел, методы стараются записать всю доступную им информацию для пользователя, чтобы проше можно было понять, что же пошло не так. И `Assert.True()` записывает буквально следующее:
...
Не особо информативно. Так что перед написанием какой-то своей толстой проверки лучше посмотреть в документацию.
Замечание 1: Обратите внимание на _Expected_ и _Actual_ в выводе. Некоторые `assert`-ы (методы класса `Assert`) выводят информацию о ожидаемых и полученных значениях (например, `Assert.Equal()`). Обычно подразумевается, что первый параметр - ожидаемый (или неожидаемый в случае `Assert.NotEqual()`). Опять же для лучшего понимания, почему тест упал, лучше этого порядка придерживаться.
Замечание 2: Некоторые `assert`-ы имеют перегруженные варианты, которые принимают дополнительное строковое сообщение, которые выводиться, если тест свалился.

Окей. А если я захочу провести один и тот же тест с разными данными. Макконнелл советует разбивает возможные данные на классы эквивалентности и выполнять тесты для данных, которые нормальными и граничными для каждого класса. Писать новый тест для каждого набора входных данных - не очень хорошая идея. При подготовке теста создать коллекцию с возможными данными, а в тесте по ней пройтись и проверить каждый вариант. Казалось бы лучше, но из-за засовывания нескольких тестов в один у нас появляются следующие проблемы:

1. Если непоследний набор упадет, мы не узнаем, оставшиеся прошли или нет.
2. В некоторых ситуациях сложно понять, на каком наборе тест провалился (если при выполнении теста упало исключение).
   Так что этот вариант даже хуже. Можно, конечно, оборачивать тест каждого набора в `try-catch` и самостоятельно делать дополнительный вывод, но зачем? Есть такая замечательная штука, как `Theory`. Это аннотация, которой помечаются специальные тесты, которые принимают параметры. А способ получения параметров описывается в `XxxData` аннотациях (`InlineData`, `ClassData` и `MemberData`). Вот как это выглядит:
